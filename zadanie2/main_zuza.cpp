...
int main() {
   ...
   // x[n-1] = b;
    ...
    for(int i=0; i<n; i++)
    {
      ...
      //  std::cout<<"k[0] "<<k[0]<<" k[1]: "<<k[1]<<" k[2]: "<<k[2]<<" k[3]: "<<k[3]<<std::endl;
    ...
    for(int i=0; i<n; i++)
    {
        ...
        y2[i+1] = 1.0 * y2[i] + ((25*j[0]/216 + 1408*j[2]/2565 + 2197 *j[3]/4104 - j[4]/5) * h);
    }
    ...
    for(int i=0; i<n; i++)
    {
        ...
        std::cout<<"x: "<<x[i]<<" y: "<<y[i]<<std::endl;
        y2[i+1] = 1.0 * y2[i] + ((16.0*l[0]/135.0) + (6656.0*l[2]/12825.0) + (28561.0 *l[3]/56430.0) - (9.0*l[4]/5.0) + (2.0*l[5]/55.0)) * h;
    }
    std::cout<<"x: "<<x[n]<<" y: "<<y[n]<<std::endl;
    return 0;
}
